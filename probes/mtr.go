...
func Mtr(cd *Probe, triggered bool) (MtrResult, error) {
	...
	case "linux":
		...
		cmd = exec.CommandContext(ctx, "/bin/bash", args...)*/
		args := []string{"-c", "./lib/trip_linux64 --icmp --mode json --multipath-strategy paris --dns-resolve-method cloudflare --report-cycles " + strconv.Itoa(triggeredCount) + " --dns-lookup-as-info " + cd.Config.Target[0].Target}
		cmd = exec.CommandContext(ctx, "/bin/bash", args...)
		break
	default:
